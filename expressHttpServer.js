...
app.get('/authResult', function(req, res){
  ...
  var option = {
    ...
    form : {
        code : authCode,
        client_id : '9Kz1OVBYF84FZQ8sduwucODoNFEgf9rDgAjowJBB',
        client_secret : '4WYxBf6wJwu2RVpazDuHjfe7oTsOc3DbEHZHj6jB',
...
app.post('/login', function(req, res){
  ...
  connection.query(sql, [userEmail], function(err, results, fields){
    if(err){
      console.error(err);
      throw err;
    }
...
app.post('/balance', auth, function(req, res){
  ...
  var transId = "T991608460U" + countnum;
  var sql = "SELECT * FROM user WHERE id = ?"
  connection.query(sql,[user.userId], function (err, results, fields) {
    var option = {
      method : "GET",
...
app.post('/account', auth, function(req, res){
  var user = req.decoded;
  console.log(user.userName + " 접속하여 거래내역 조회를 합니다.");
  var finusenum = req.body.fin_use_num;
  var countnum = Math.floor(Math.random() * 1000000000) + 1;
  var transId = "T991608460U" + countnum;
  var sql = "SELECT * FROM user WHERE id = ?"
...
app.post('/withdraw', auth, function(req, res){
  var user = req.decoded;
  console.log(user.userName + " 접속하여 출금이체를 합니다.");
 // var finusenum = req.body.fin_use_num;
  var countnum = Math.floor(Math.random() * 1000000000) + 1;
  var transId = "T991608460U" + countnum;
  ...
  connection.query(sql,[user.userId], function (err, results, fields) {
    var option = {
      ...
      json : {
          bank_tran_id: transId,
          cntr_account_type: "N",
          cntr_account_num: "2978372479",
          dps_print_content: "쇼핑몰환불",
          fintech_use_num: "199160846057881806122904",
          wd_print_content: "쇼핑몰환불",
          tran_amt: amount,
          req_client_fintech_use_num: "199160846057881806122904",
          tran_dtime: "20200205172120",
          req_client_name: "손민주",
          req_client_num: "2978372479",
          transfer_purpose: "TR",
          recv_client_name: "손민주",
          recv_client_bank_code: "097",
          recv_client_account_num: "2978372479"
      }
    }
    request(option, function (error, response, body) {
      console.log(body);
